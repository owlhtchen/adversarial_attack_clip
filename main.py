...
r = (torch.randn(image.size(), dtype=image.dtype, device=device) * 0.1).requires_grad_()
...
for i in range(epochs):
    optimizer.zero_grad()
    messup_logits_per_image, _ = model(image + r, text)
...
save_to(image + r.detach(), "mess.jpg")
